...
public class PacienteDAOH2 implements Idao<Paciente>{
    ...
    private static final String SQL_INSERT = "INSERT INTO PACIENTE (NOMBRE, APELLIDO, DNI, FECHA_INGRESO, DOMICILIO_ID, EMAIL) VALUES (?, ?, ?, ?, ?, ?);";
    private static final String SQL_UPDATE = "UPDATE PACIENTE SET NOMBRE = ?, APELLIDO = ?, DNI = ?, FECHA_INGRESO = ?, DOMICILIO_ID = ?, EMAIL = ? WHERE ID = ?;";
    ...
    public List<Paciente> search() {
        ...
        try {
            ...
            while (rs.next()){
                Domicilio domicilio = domAux.searchID(rs.getInt(6));
                aux = new Paciente(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDate(5).toLocalDate(), domicilio, rs.getString(7));
    ...
    @Override
    public Paciente searchString(String value) {
        LOGGER.info("Searching paciente string ...");
        Connection con = null;
        Paciente paciente = null;
        try{
            con = DB.getConnection();
            PreparedStatement pStm = con.prepareStatement(SQL_SELECT_EMAIL);
            ...
            LOGGER.info("Paciente string found");
        }
        catch (Exception e){
            LOGGER.error("Paciente string search failed " + e.getMessage());
        }
        finally {
            try{
                con.close();
                ...
                LOGGER.error("Connection closing during paciente searchString failed: " + e.getMessage());
            }
        }
        return  paciente;
    }
